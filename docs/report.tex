...
Representing real-valued signals and transmitting them efficiently without 
loss of accuracy is a key process of the information age. One important method 
of representing a time- (or space-) dependent signal which inherently sorts 
out the significant data is by representing the signal in the frequency domain 
and transmitting with that format. The data set for a frequency-domain signal 
can be cut down by removing insignificant data where the frequency is too high 
to be perceived by humans. This paper investigates the methods of the Discrete 
Fourier Transform (DFT) and its algorithmically advantageous brother, the Fast 
Fourier Transform (FFT), and their use in transforming discrete sampled 
signals into the frequency domain, where the signal can be more easily analyze.
Furthermore, we discuss the practical and mathematical significance of using 
the FFT in Frequency Division Multiplexing (FDM), over other earlier 
techniques, and demonstrate how to digitally process and transmit a sound 
signal using this method, similar to the manner in which telephone 
